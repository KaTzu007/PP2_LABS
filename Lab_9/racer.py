...
score = 0
coin = 0
threshold = 0 #Для отслеживания монет
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == incSpeed: #Пользовательское событие
            if speedForCoin <= 18: #Лимит на скорость
                speedForCoin += 1
                speedForCar += 0.5
    ...
    if pygame.sprite.spritecollideany(P1, coinsGroup): #Проверка на взаимодействие между игрока с монетой
        ...
        weight = random.randint(1, 3) #Рандомное число от 1 до 3
        coin += weight #Добавляем weight к общему колчиеству монет

        if coin >= threshold + 5: #Для отслеживания предела
            threshold += 5
            if speedForEnemy <= 18:
                speedForEnemy += 1 
            #В итоге добавляем скорость врагу, если количество монет превысила предел монет кратных 5(5, 10, 15, ...)
